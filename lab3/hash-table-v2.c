...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int ec=pthread_mutex_init(&(entry->lock),NULL);
	        if (ec!=0)
	        {
		  exit(ec);
	        }
                             ...
                             uint32_t value)
{
	...
	list_entry->key = key;
	list_entry->value = value;
	int ec=pthread_mutex_lock(&(hash_table_entry->lock));
	if (ec!=0)
	{
	  exit(ec);
	}
	SLIST_INSERT_HEAD(list_head, list_entry, pointers);
	ec=pthread_mutex_unlock(&(hash_table_entry->lock));
	if (ec!=0)
	{
	  exit(ec);
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
		int ec=pthread_mutex_destroy(&(entry->lock));
		if (ec!=0)
		{
		  exit(ec);
		}
